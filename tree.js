...
const ClassificationHelper = {
    // Get all unique classes from our animals
    getClasses: function () {
        const classes = [];
    ...
    // Get all orders in a specific class
    getOrdersByClass: function (className) {
        const orders = [];
        for (let i = 0; i < animalDatabase.length; i++) {
            const animal = animalDatabase[i];
            if (animal.classification.class === className) {
                const orderName = animal.classification.order;
    ...
    // Get all families in a specific order
    ...
    countAnimalsInClass: function (className) {
        let count = 0;  // ← THIS IS THE CRITICAL LINE
        for (let i = 0; i < animalDatabase.length; i++) {
...
function showAnimals(familyName, orderName, className) {
    ...
    for (let i = 0; i < animals.length; i++) {
        ...
        html += '<img src="' + animal.imageUrl + '" alt="' + animal.name + '">';
